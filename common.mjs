...
function getCountBy(events, getKey, getValue) {
  ...
  for (const event of events) {
    ...
    totals[key] = (totals[key] || 0) + getValue(event);
  }
...
function getMostListenedSong(events) {
  return getMostFrequent(countBy(events, (event) => event.song_id));
}
...
function getMostListenedArtist(events, getSong) {
  return getMostFrequent(
    countBy(events, (event) => getSong(event.song_id).artist),
  );
}

// Friday night events
function getFridayNightEvents(events) {
  return events.filter((event) => isFridayNight(event.timestamp));
}
...
function getSongsListenedEveryDay(events) {
  ...
  return Object.keys(songDays).filter(
    (songID) => songDays[songID].size === allDays.size,
  );
}
// Top genres
function getTopGenres(events, getSong) {
  const counts = countBy(events, (event) => getSong(event.song_id).genre);
